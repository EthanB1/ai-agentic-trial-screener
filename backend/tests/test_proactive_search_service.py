...
async def test_run_proactive_search(proactive_search_service, mock_db, app):
    # Mock methods
    proactive_search_service.fetch_new_trials = AsyncMock(return_value=[{"nct_id": "NCT12345678"}])
    proactive_search_service.process_and_save_trials = AsyncMock(return_value=[{"nct_id": "NCT12345678"}])
    ...
    proactive_search_service.fetch_new_trials.assert_called_once()
    proactive_search_service.process_and_save_trials.assert_called_once()
    proactive_search_service.match_trials_with_patients.assert_called_once()
    proactive_search_service.update_last_check_date.assert_called_once()
...
async def test_match_trials_with_patients(proactive_search_service, mock_db, app):
    ...
    # Create an AsyncIterator with mock patients
    mock_cursor = AsyncIterator(mock_patients.copy())

    # Mock the find method to return our AsyncIterator
    mock_db.patients.find = AsyncMock(return_value=mock_cursor)
    ...
    proactive_search_service.trial_matching_service.match_patient_to_trials.assert_called_once_with(
        str(mock_patients[0]['_id']), mock_trials
    )
    proactive_search_service.notify_patient.assert_called_once_with(mock_patients[0], mock_matches)
    proactive_search_service.update_patient_matches.assert_called_once_with(mock_patients[0]['_id'], mock_matches)
...
async def test_notify_patient(proactive_search_service, app, mock_db):
    ...
    proactive_search_service.update_patient_notification = AsyncMock()
    ...
    proactive_search_service.update_patient_notification.assert_called_once()
